...
What would it look like if it was tidy?
...
-   define data & map aesthetics (also, omitting argument names)
    -   data = penguins (or the data frame name)
    -   aes (aesthetics call), label x = bill length, y = bill depth, color by species
-   add a geometry layer
    -   how we make the scatter plot (geom_point)
-   update how aesthetic mappings manifest visually (i.e. scaling)
-   map custom colors
    -   scale_color_manual: we are manually putting 3 values for each species
...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = species)) +
  geom_point() +
  scale_color_manual(values = c("lightgreen", "violet","cyan4" ))
...
ggplot() + 
  ...
  geom_point(data = penguins_summary, mapping = aes(x = mean_bill_length_mm, 
                                                    ...
                                                    fill= species), 
             size = 5, shape = 24) #24 is triangle 
```
...
penguins |>
  ...
  ggplot(aes(x = bill_length_mm, y = bill_depth_mm, color = "blue"))+
  geom_point()
```
...
ggplot(penguins, aes (x= species)) +
  geom_bar(stat = "count") #in ?geom it says the default is stat count, so you can just do geom_bar() and get the same result
```
...
ggplot(penguin_summary, aes(x= species, y =n)) +
  geom_bar(stat = "identity") #override default stat by naming it 
```
...
ggplot(penguins, aes(x = species)) +
  geom_bar()+
  coord_cartesian() #this is the default
```
...
(p1+(p2/p3))+ 
  plot_annotation(title = "All the plots mashed together")
```
